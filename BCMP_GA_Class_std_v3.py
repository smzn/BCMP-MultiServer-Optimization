...
class BCMP_GA_Class:
    def __init__(self, N, R, K_total, npop, ngen, U, crosspb, mutpb, rank, size, comm, P):
        ...
        if self.rank == 0:
            ...
            np.savetxt('./popularity_std_'+str(self.N)+'_'+str(self.R)+'_'+str(self.K_total)+'_'+str(self.npop)+'_'+str(self.ngen)+'_'+str(self.U)+'.csv', np.array(self.popularity), delimiter=',', fmt='%d')
            np.savetxt('./distance_std_'+str(self.N)+'_'+str(self.R)+'_'+str(self.K_total)+'_'+str(self.npop)+'_'+str(self.ngen)+'_'+str(self.U)+'.csv', self.distance_matrix, delimiter=',', fmt='%.5f')
            np.savetxt('./transition_probability_std_'+str(self.N)+'_'+str(self.R)+'_'+str(self.K_total)+'_'+str(self.npop)+'_'+str(self.ngen)+'_'+str(self.U)+'.csv', self.p, delimiter=',')
        else:
            self.pool = [[]]
    ...
    def getFinalResult(self, individual):
        ...
        np.savetxt('./ga_Node_std_'+str(self.N)+'_'+str(self.R)+'_'+str(self.K_total)+'_'+str(self.npop)+'_'+str(self.ngen)+'.csv', individual, delimiter=',') #窓口数
...
#実行方法
#mpiexec -n 6 python3 BCMP_GA_Class.py 33(N) 2(R) 100(K) 18(遺伝子数) 10(世代数) 2(最大窓口数)
#mpiexec -n 6 python BCMP_GA_Class_std_v3.py 33 2 100 18 10 2