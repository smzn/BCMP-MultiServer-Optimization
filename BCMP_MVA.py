...
class BCMP_MVA:
    ...
    def __init__(self, N, R, K, mu, type_list, p, m):
        ...
        self.Pi = np.zeros((self.N, np.max(self.m), self.km))
        ...
        for idx, val in enumerate(self.combi_list):
                    ...
                    if self.type_list[n] == 3:
                        ...
                        for i in range(self.R):#k-1rを状態に変換
                            if np.min(k1v) >= 0: #全ての状態が0以上のとき(一応チェック)
                                ...
                                sum_l += Decimal(str(self.L[n, i, int(kn)])) #knを整数型に変換
                        if self.m[n] == 1: #P336 (8.43) Type-1,2,4 (m_i=1)
                            #print('n = {0}, r = {1}, k_state = {2}'.format(n,r,k_state))
                            sum_L = Decimal('1') + Decimal(str(sum_l))
                            self.T[n, r, k_state] = Decimal('1') / Decimal(str(self.mu[r, n])) * Decimal(str(sum_L))
                        if self.m[n] > 1:
                            sum_pi = 0
                            for _j in range(self.m[n]-2+1):
                                ...
                                mj_1 = self.m[n] - _j - 1
                                sum_pi += Decimal(str(mj_1)) * Decimal(str(pi8_45))
                            
                            sum_L_pi = Decimal('1') + Decimal(str(sum_l)) + Decimal(str(sum_pi))
                            m_mu = Decimal(str(self.m[n])) * Decimal(str(self.mu[r,n]))
                            self.T[n, r, k_state] = Decimal('1') / Decimal(str(m_mu)) * Decimal(str(sum_L_pi))
            ...
            for r in range(self.R):
                ...
                for n in range(self.N):
                    sum += Decimal(str(self.alpha[r][n])) * Decimal(str(self.T[n,r,k_state]))
                if sum == 0:
                    continue
                if sum > 0:
                    self.lmd[r,k_state] = Decimal(str(val[r])) / Decimal(str(sum))
            ...
            for n in range(self.N):#P336 (8.47)
                for r in range(self.R):
                    self.L[n,r,k_state] = Decimal(str(self.lmd[r,k_state])) * Decimal(str(self.T[n,r,k_state])) * Decimal(str(self.alpha[r][n]))
    ...
    def getPi(self, n, j, k, kr):
        ...
        if j == 0 and sum(kkr) > 0: #(8.45)
            ...
            for _r in range(self.R):
                sum_emlam += Decimal(str(self.alpha[_r][n])) / Decimal(str(self.mu[_r][n])) * Decimal(str(self.lmd[_r][state_number]))
            sum_pi = 0
            for _j in range(1, self.m[n]):
                pi8_44 = self.getPi8_44(n, _j, kkr, state_number) #このgetPiは人数を減らさない
                self.Pi[n][_j][state_number] = pi8_44
                mj = self.m[n] - _j
                sum_pi += Decimal(str(mj)) * Decimal(str(pi8_44))

            sum_sum = Decimal(str(sum_emlam)) + Decimal(str(sum_pi))
            m = Decimal('1') / Decimal(str(self.m[n]))
            pi = Decimal('1') - Decimal(str(m)) * Decimal(str(sum_sum))
            return pi
    ...
    def getPi8_44(self, n, j, k, state_number): #(8.45)から(8.44)を呼び出すときだけ利用 (人数を減らさず呼び出し)
        ...
        for _r in range(self.R):
            ...
            if kr_state_number < 0:
                ...
                if self.Pi[n][j-1][kr_state_number] == 0 and kr_state_number == 0: #必要ないかも
                    sum_val += 0
                elif self.Pi[n][j-1][kr_state_number] == 0 and kr_state_number > 0: #必要ないかも
                    pi8_45 = self.getPi(n, j-1, k, kr)
                    sum_val += Decimal(str(self.alpha[_r][n])) * Decimal(str(self.lmd[_r][state_number])) * Decimal(str(pi8_45)) / Decimal(str(self.mu[_r][n]))
                else:
                    sum_val += Decimal(str(self.alpha[_r][n])) * Decimal(str(self.lmd[_r][state_number])) * Decimal(str(self.Pi[n][j-1][kr_state_number])) / Decimal(str(self.mu[_r][n]))


        pi = Decimal('1') / Decimal(str(j)) * Decimal(str(sum_val))
        return pi
...
if __name__ == '__main__':
    ...
    p = pd.read_csv('./alpha_N10_R2_K50_Core16.csv').values.tolist()
    #alpha = pd.read_csv('./alpha_N33_R2_K500_Core16.csv').values.tolist() 
