...
class BCMP_GA_Class:
    def __init__(self, N, R, K_total, npop, ngen, U, crosspb, mutpb, rank, size, comm, w, transition_matrix):
        self.N = N
        self.R = R
        ...
        self.m = randint(1, U+1, N) #ランダムな値を取るように改変
        self.npop = npop
        self.ngen = ngen
        self.U = U #最大窓口数
    ...
    def genetic_algorithm(self): #objective, n_bits, n_iter, n_pop, r_cross, r_mut
        ...
        for gen in range(self.ngen):
            if self.rank == 0:
                with open(self.process_text, 'a') as f:
                    print('{0}世代目計算開始'.format(gen), file=f)
            ...
            for pop_index in range(self.rank, self.npop, self.size):#自分の担当遺伝子だけループ 
                ...
                with open(self.process_text, 'a') as f:
                    print('各プロセスでの遺伝子計算', file=f)
                    print ("rank = {1}, pop_id = {2}, calclation_time:{0}".format(elapsed_time, self.rank, pop_index) + "[sec]", file=f)
                    print('pop_index = {0}, scores = {1}, rank = {2}'.format(pop_index, self.scores[pop_index], self.rank), file=f)
            ...
            if self.rank == 0:
                for i in range(1, self.size):
                    ...
                    with open(self.process_text, 'a') as f:
                        print('遺伝子receive : {0}, {1}'.format(i, scores), file=f)
                    #リストの結合
            ...
            if self.rank == 0:
                with open(self.process_text, 'a') as f:
                    print('遺伝子集約完了：Aggregation scores', file=f)
            ...
            if self.rank == 0:
                for i in range(self.npop):
                    if self.scores[i] < best_eval: #最小値を探す
                        ...
                        with open(self.process_text, 'a') as f:
                            print("{0}世代, new best {1} = {2}".format(gen, self.pool[i], self.scores[i]),file=f)
                            print('窓口利用数 : {0}'.format(sum(self.pool[i])), file=f)
                # select parents
            ...
            with open(self.process_text, 'a') as f:
                print('次世代遺伝子同期(rank = {0}) : {1}'.format(self.rank, self.pool),file=f)
            
        if self.rank == 0:
            self.getGraph()
            #20211230 Final Result
            with open(self.process_text, 'a') as f:
                print('最終結果の計算', file=f)
...
#        return distance_matrix
   ...
   #重力モデルで推移確率行列を作成 
    ...
    def getObjective(self, l): #目的関数に窓口数によるコストを追加
        l = np.array(l)
        l1 = l.reshape(1,-1)
        val = np.std(l1)
        #return val
...
if __name__ == '__main__':
    ...
    mutpb = 0.2
    start = time.time()
    bcmp = BCMP_GA_Class(N, R, K_total, npop, ngen, U, crosspb, mutpb, rank, size, comm, w, transition_matrix)
...
#実行方法
#mpiexec -n 6 python3 BCMP_GA_Class_std_v4.py 33(N) 2(R) 100(K) 18(遺伝子数) 10(世代数) 2(最大窓口数) 1(窓口数コストウエイト)
#mpiexec -n 6 python BCMP_GA_Class_std_v4.py 33 2 100 18 10 2 1